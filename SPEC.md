...
A private website where Alex's daughter can see all her family — faces, names, relationships, locations — across multiple countries. Family members join via invite link, optionally link their Facebook for profile enrichment, and browse an interactive family tree. Privacy is enforced by role (Phase 1) and graph-distance (Phase 2). No ads. No algorithms. No Zuckerberg.
...
- Ancestry paths ("Mia → Maria → Бабушка Юки → ...")
...
| branch | str | no | null | max 100 | Display hint: "rivera", "santos", "maternal". Presentation only — not truth. |
...
|-------|------|----------|---------|-------------|-------|
| id | UUID | yes | auto | PK | |
| name | str | yes | — | max 200 | "Mom's caregiving robot", "Alex's assistant" |
...
3. Admin sends link via WhatsApp/Telegram/email: `https://family.example.com/invite/{token}`
...
4. If email doesn't match → "Ask Alex to invite you" message
...
4. If no match: reject with "Ask Alex to add you first" (never auto-create unlinked people)
...
**Flow:** Envelope receives email to family@family.example.com → calls this webhook → Family Book extracts photos from attachments, matches sender email to Person, creates Moments.
...
1. **Parasite:** Ingest from everywhere. WhatsApp export, Facebook export, email forwarding, share sheet. Fill the archive with EXISTING content. Family changes zero behavior. Alex does the setup once.
...
3. **Better:** The feed, tree, birthday reminders, agent API — these become reasons people start posting directly. Not because Alex forced them, but because it's genuinely better.

**Operational model:** Zero-touch. Alex sets up ingestion once. Content flows in automatically. Alex intervenes only for exceptions (merge conflicts, new family members, relationship changes). If Alex has to manually run exports every week, the archive falls behind and dies.
...
|-----------|-----------|-------------|
| Admin (Alex/Maria) | `is_admin = true` | Dashboard: new content, pending imports, system health, quick actions |
...
├─────────────────────────────────┤
│  ┌─────────────────────────────┐│
│  │ 📷  Alex Rivera            ││  ← source person
│  │     2h ago · via WhatsApp · Barcelona ││  ← time + source + location
│  │                             ││
...
```json
{
  "id": "uuid",
  "kind": "photo",
  "poster": { "id": "uuid", "display_name": "Alex Rivera", "photo_url": "/photos/..." },
  "about": { "id": "uuid", "display_name": "Mia" },
...
Content flows INTO Family Book through multiple automated pipelines. Alex sets up each pipeline once. After that, it runs unattended.

| Pipeline | Automation Level | How It Works | Alex's Setup Work |
|----------|-----------------|--------------|-------------------|
| **Email** (Envelope) | Fully automatic | family@family.example.com receives forwarded photos. Envelope parses sender, extracts images, creates Moments. | Configure Envelope route (once) |
| **WhatsApp group export** | Semi-automatic | Alex exports family group chat with media. Upload .zip → parser extracts all photos + captions + sender mapping. | Export + upload (~15 min/quarter) |
| **Facebook data export** | Semi-automatic | Family members request FB data export. Forward download email to family@family.example.com. Envelope catches it, ingestion is automatic. | Tell family members once |
...
services:
  ...
  conduit:
    ...
    environment:
      CONDUIT_SERVER_NAME: family.example.com
...
Admin (Alex) configures push preferences for each family member. Family members can later adjust their own via profile settings.
...
- Alex sees both reactions appear in the Family Book feed
...
- **Color palette:** Branch-coded. Rivera = `#4A90D9`, Santos = `#6BBF6B`, Maternal = `#D94A4A`, Shared = `#9B59B6`.
...
If Alex wants to add a new locale (e.g., Arabic, French), he can run a CLI script:
```bash
python scripts/generate_locale.py --locale fr --source locales/en.json
```
This calls OpenRouter to generate a draft translation, writes it to `locales/fr.json`, and Alex reviews + edits before committing. **Never called at runtime. Never in production.**
...
```json
{
  "participants": [{"name": "Alex Rivera"}, {"name": "Maria Rivera"}],
  "messages": [
    {
...
3. They forward the email to `family@family.example.com`
...
1. Alex opens the family WhatsApp group → Settings → Export Chat → **"Include Media"**
...
3. Alex uploads the `.zip` to Family Book admin panel (`/admin/import/whatsapp`)
4. Parser extracts:
...
5. Admin mapping UI: Alex sees a list of extracted contact names and matches each to a Person record (existing or new)
...
```
[15/03/2026, 14:30:00] Alex: This is the first line
of a message that spans
multiple lines

[15/03/2026, 14:31:00] Maria: And this is a new message
```
...
```
# iOS style:
[15/03/2026, 14:30:00] Alex: <attached: IMG-20260315-WA0042.jpg>

# Android style:  
15/03/26, 14:30 - Alex: IMG-20260315-WA0042.jpg (file attached)

# With caption (iOS):
[15/03/2026, 14:30:00] Alex: Look at this! <attached: IMG-20260315-WA0042.jpg>

# With caption (Android):
15/03/26, 14:30 - Alex: Look at this!
15/03/26, 14:30 - Alex: IMG-20260315-WA0042.jpg (file attached)
```
...
```
[15/03/2026, 14:30:00] Alex added Dmitri
[15/03/2026, 14:31:00] Dmitri left
[15/03/2026, 14:32:00] Alex changed the group description
[15/03/2026, 14:33:00] Alex changed this group's icon
...
[15/03/2026, 14:35:00] Alex changed the subject from "Family" to "Rivera Family"
...
```
[15/03/2026, 14:37:00] Alex: This message was deleted
[15/03/2026, 14:37:00] Alex: You deleted this message
```
...
```
[15/03/2026, 14:30:00] Alex: <attached: 00000042.vcf>
[15/03/2026, 14:31:00] Alex: location: https://maps.google.com/?q=40.4168,-3.7038
```
...
```
[15/03/2026, 14:30:00] Alex: <attached: PTT-20260315-WA0001.opus>
[15/03/2026, 14:31:00] Alex: <attached: AUD-20260315-WA0001.mp3>
```
...
When Alex exports the same group multiple times (quarterly), photos will overlap.
...
- **No phone numbers in export.** Contact names only. Alex must manually map names to people.
...
```
WhatsApp Import: "Rivera Family Group" — 847 messages, 312 photos, 23 videos

Contact Name Mapping:
┌─────────────────────┬──────────────────────────────┬──────────┐
│ WhatsApp Name        │ Match To                     │ Status   │
├─────────────────────┼──────────────────────────────┼──────────┤
│ Alex                │ [Alex Rivera ▾] (auto)      │ ✅ Mapped │
│ Maria               │ [Maria Rivera ▾] (auto)     │ ✅ Mapped │
│ Мама                 │ [Create new person ▾]        │ ⚠️ New    │
│ Amy              │ [Amy Rivera ▾] (auto)    │ ✅ Mapped │
...
- Requires: `wacli` configured with Alex's WhatsApp, phone numbers in Person.contact_whatsapp
...
- `family.example.com` has the Rivera/Santos family SQLite
- `garcia.family` has the Garcia family SQLite
- When a Rivera marries a Garcia, both admins agree to federate
...
| BASE_URL | str | yes | — | Public URL: `https://family.example.com` or `https://family-book.up.railway.app` |
...
| MATRIX_BOT_USER | str | no | — | e.g., `@familybook:family.example.com` |
...
Phase 1: Railway auto-subdomain is fine. Alex decides the final domain (`family.example.com` or other) before sending invites to family.
...
1. **Alex:** Export the family WhatsApp group chat with media (~2 minutes)
2. **Alex + Maria:** Map WhatsApp contact names to real family members in admin UI (~30 min)
3. **Alex + Maria:** Add relationship edges (parent-child, partnerships) for mapped people (~1 evening)
4. **Alex:** Decide domain (family.example.com or other)
5. **Alex:** Configure SMTP or Envelope for magic link emails
6. **Alex + Maria:** Send invite links to family
7. **Alex:** Register Facebook Developer App (~30 min, only if enabling Facebook)
8. **Alex + Maria:** Curate Russian relationship terms (for i18n)
...
| Smart home | "Let Amy in" — who is Amy? Is she authorized? | Family membership + relationship = implicit access control |
...
```json
{
  "person": {
    ...
    "residence": { "place": "Barcelona, Spain", "country_code": "ES", "timezone": "Europe/Barcelona" },
    "age": 44
  },
  "household": [
    { "name": "Maria Rivera", "relationship": "wife", "languages": ["ru", "en", "es"] },
    { "name": "[redacted]", "relationship": "daughter", "age": 4 }
  ],
  "emergency_contacts": [
    { "name": "Maria Rivera", "relationship": "wife", "phone": "+34...", "priority": 1 },
    { "name": "Father Rivera", "relationship": "father", "phone": "+1...", "priority": 2, "timezone": "America/Portland" }
  ],
  "upcoming": [
  ...
  "branches": ["rivera", "santos", "maternal"]
}
```
...
- No claim of "self-hosted" when running on Railway — it's Alex's account, not Alex's hardware
